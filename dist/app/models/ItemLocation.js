...
class ItemLocation {
    constructor({ latitude, longitude, lostItemId, foundItemId, messageId = null, }) {
    ...
    save() {
        let sql = 'INSERT INTO itemslocations(latitude,longitude,lostItemId,foundItemId,messageId) VALUES(?,?,?,?,?)';
    ...
    static getLocation({ itemId }) {
        let sql = 'SELECT * FROM itemslocations WHERE lostItemId=? OR foundItemId=?';
        return promisePool.execute(sql, [itemId, itemId]);
    }
}
